...
public class getConversation {
    ...
    static getConversation singleton;

    public static getConversation getSingleton(){
        if(singleton==null){
            singleton=new getConversation();
        }
        return singleton;
    }

    public File process(InputStream input, Long length){
        ...
        try{
            while(true){
                ...
                if(entry.length!=2){
                    return new File("G:\\java\\os_lab8\\server\\html\\login_illegal.html");
                }
    ...
    public static File apply(InputStream input,Long length){
        return getSingleton().process(input, length);
    }
}
